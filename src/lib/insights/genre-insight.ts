...
export function getGenreInsight(genres: GenreStats[]): string {
  ...
  if (g2 && g2.percentage > 25 && ratio12 < 1.6 && top2 > 58) {
    return pick([
      `${g1.name} et ${g2.name} se partagent la tête — t'as vraiment un pied dans les deux. Difficile de dire lequel domine.`,
      `Deux genres au même niveau : ${g1.name} et ${g2.name}. Selon les humeurs probablement.`,
      `T'as autant d'affinité pour le ${g1.name} que pour le ${g2.name} — les deux ressortent vraiment à des niveaux similaires.`,
  ...
  if (n > 6 && g1.percentage > 30 && othersPercentage > 28) {
    return pick([
      `Le ${g1.name} c'est ton port d'attache, mais t'écoutes beaucoup d'autres choses aussi. ${n} genres dans ton historique.`,
      `Tu reviens souvent au ${g1.name}, mais sans te limiter à ça. Beaucoup de styles différents dans tes écoutes.`,
      `${g1.name} en tête, et une vraie curiosité derrière — ${n} genres au total, t'es pas du genre à rester dans une seule case.`,
